...
async function readBody(req) {
  ...
  for await (const chunk of req) {
    bytes += chunk.length;
    if (bytes > 300000) throw new InputError("请求体过大");
    chunks.push(chunk);
  }
  try {
  ...
  return http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (req.method === "DELETE" && path === "/api/session") {
        if (s.busy) return send(res, 409, { error: "请等待当前模型请求结束" });
        sessions.delete(sid);
        res.setHeader(
          "set-cookie",
      ...
      if (path.startsWith("/api/models")) {
        ...
        if (path.endsWith("/list")) {
          s.busy = true;
          try {
            return send(res, 200, await listModels(config, options));
          } finally {
            s.busy = false;
          }
        }
        if (path.endsWith("/test-structure")) {
          ...
          try {
            ...
            const result = await testStructure(config, options);
      ...
      s.busy = true;
      try {
        const result = path.endsWith("/prepare")
          ? await prepareInterview(input, s.config, options)
          : await analyzeInterview(input, s.config, options);
        const data = {
          ...result,
